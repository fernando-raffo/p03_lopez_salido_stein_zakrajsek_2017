"""
Process raw FRED data pulled by `pull_fred.py` into the series used in
the replication.
"""
...
# Maps each column of the cleaned DataFrame returned by `clean_fred_data`
# to a human-readable description of how it is constructed. Used by
# `save_data_readme` to document `fred_final_series.parquet`.
column_descriptions = {
    "GDP": (
    ...
    "BAA": (
        ...
        "`final_baa_series` as the December value of the monthly BAA "
        "series."
    ),
    "AAA": (
    ...
    "Treasury_10yr": (
        ...
        "computed by `final_10yr_treasury_series` as the December value "
        "of a monthly series combining GS10, M1333BUSM156NNBR, and "
...
def save_data_readme(df, data_dir=PROCESSED_DATA_DIR):
    ...
    ----------
    df : pandas.DataFrame
        The DataFrame returned by `clean_fred_data`.
    data_dir : str or Path, default PROCESSED_DATA_DIR
        Directory to write `fred_final_series_readme.md` into.
    ...
    file_path = filedir / "fred_final_series_readme.md"

    lines = [
        "# FRED Cleaned Series README",
        "",
        "This file documents the columns found in `fred_final_series.parquet`, "
        "generated by `process_fred_data.py` from the raw series pulled by "
        "`pull_fred.py`.",
        "",
        "| Column | Description |",
        "| --- | --- |",
...
if __name__ == "__main__":
    ...
    cleaned_df = clean_fred_data(fred_df)
    filedir = Path(PROCESSED_DATA_DIR)
    filedir.mkdir(parents=True, exist_ok=True)
    cleaned_df.to_parquet(filedir / "fred_final_series.parquet")
